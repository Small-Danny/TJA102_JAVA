-- 1.寫一個SQL語句，創建一張會員資料表 member_info，欄位有 member_id, account, password, member_name, address 以及 email，以下是詳細資訊:
